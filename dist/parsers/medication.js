...
function getFrequency(req) {
    const di = req.dosageInstruction?.[0];
    if (!di)
        return '';
    const repeat = di.timing?.repeat;
    if (!repeat)
        return di.timing?.code?.text ?? '';
    const { frequency, period, periodUnit } = repeat;
    if (frequency && period && periodUnit) {
        const unitMap = { d: 'day', wk: 'week', mo: 'month', h: 'hour' };
        const unit = unitMap[periodUnit] ?? periodUnit;
        return frequency === 1 ? `every ${period} ${unit}` : `${frequency}x / ${period} ${unit}`;
    }
    return '';
}
function getPrescriber(req) {
    return req.requester?.display;
}
function classify(name) {
    const lower = name.toLowerCase();
    if (CHEMO_DRUG_KEYWORDS.some(k => lower.includes(k)))
        return 'chemo';
    if (PROPHYLAXIS_KEYWORDS.some(k => lower.includes(k)))
        return 'prophylaxis';
    if (SUPPORTIVE_KEYWORDS.some(k => lower.includes(k)))
        return 'supportive';
    return 'other';
}
export function parseMedicationRegimen(requests) {
    const regimen = {
        chemoAgents: [],
        supportive: [],
        prophylaxis: [],
    ...
    for (const req of requests) {
        ...
        const isPrn = req.dosageInstruction?.some(di => di.text?.toLowerCase().includes('as needed') || di.text?.toLowerCase().includes('prn'));
        if (isPrn) {
        ...
        if (group === 'chemo')
            regimen.chemoAgents.push(item);
        else if (group === 'supportive')
            regimen.supportive.push(item);
        else if (group === 'prophylaxis')
            regimen.prophylaxis.push(item);
        else
            regimen.other.push(item);
    }
    return regimen;
}
