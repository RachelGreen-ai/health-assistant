export function parseAppointments(appointments, distanceThresholdMiles) {
    return appointments.map(appt => {
        ...
        const providerRef = appt.participant?.find(p => p.actor?.reference?.startsWith('Practitioner'));
        ...
        const locationRef = appt.participant?.find(p => p.actor?.reference?.startsWith('Location'));
