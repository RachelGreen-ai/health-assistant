...
import type { TreatmentEvent, TreatmentEventType } from '../types/domain.js';
...
function classifyProcedure(display: string): TreatmentEventType {
  const d = display.toLowerCase();
  if (TRANSPLANT_KEYWORDS.some(k => d.includes(k))) return 'transplant';
  if (CHEMO_KEYWORDS.some(k => d.includes(k)))      return 'chemo_cycle';
  if (RADIATION_KEYWORDS.some(k => d.includes(k)))  return 'radiation';
  if (INFUSION_KEYWORDS.some(k => d.includes(k)))   return 'infusion';
  if (SURGERY_KEYWORDS.some(k => d.includes(k)))    return 'surgery';
  return 'procedure';
}

export function parseProcedures(procedures: FhirProcedure[]): TreatmentEvent[] {
  const events: TreatmentEvent[] = procedures.map(proc => {
