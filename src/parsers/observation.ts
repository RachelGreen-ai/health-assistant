...
import type { LabResult, LabTrends, LabInterpretation } from '../types/domain.js';
import { LOINC_DISPLAY } from '../constants/loinc.js';
import { NoDataError } from '../types/domain.js';

function mapInterpretation(codes: string[]): LabInterpretation {
  const c = codes.map(s => s.toUpperCase());
  if (c.some(s => s === 'HH' || s === 'LL' || s === 'AA')) return c.includes('HH') ? 'critical-high' : 'critical-low';
  if (c.some(s => s === 'H'))  return 'high';
  if (c.some(s => s === 'L'))  return 'low';
  if (c.some(s => s === 'N' || s === 'NORMAL')) return 'normal';
  return 'unknown';
}

export function parseLabTrends(observations: FhirObservation[], panelName: string): LabTrends {
  if (!observations.length) {
    throw new NoDataError(`No ${panelName} lab results found for the requested period.`);
  }

  // Group by LOINC code
  const byCode: Record<string, LabResult[]> = {};
  ...
  for (const obs of observations) {
    ...
    if (refRange?.text) {
      ...
      if (lo && hi) referenceRange = `${lo} – ${hi}`;
      else if (lo)  referenceRange = `≥ ${lo}`;
      else if (hi)  referenceRange = `≤ ${hi}`;
    }
    ...
    const result: LabResult = { loincCode: code, display, value, unit, referenceRange, interpretation, effectiveDate };

    if (!byCode[display]) byCode[display] = [];
...
export function parseLatestLabs(observations: FhirObservation[]): LabResult[] {
